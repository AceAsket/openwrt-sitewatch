...
Пассивные наблюдения дополнительно агрегируются в **Сетевой отчет**: `источник + протокол + IP + порт`, сумма пакетов/байт, число сэмплов и последний статус. Из отчета можно скачать:
