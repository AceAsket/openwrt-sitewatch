...
SITEWATCH_AGENT_INGEST_URL="<container-host>:8095"
SITEWATCH_INGEST_TOKEN="<same-token>"
```

То же самое можно задать в веб-интерфейсе роутера: вкладка **Настройки** -> режим **Агент в контейнер**, адрес контейнера в формате `host:port` и общий токен. После этого обычная кнопка запуска окна наблюдения на роутере будет стартовать `sitewatch-agent`, а не локальный сборщик.
...
CGI-интерфейс пока остается основным совместимым UI для OpenWrt, но новая архитектура постепенно переносит состояние и фоновые операции в Go. Экспериментальный API-сервер можно запустить так:

```sh
sitewatch serve :8095
```

Доступные endpoints:

- `GET /api/status` - состояние capture/scan/detector и счетчики;
- `GET /api/domains?source=192.168.50.80` - строки доменной таблицы;
- `GET /api/network?source=192.168.50.80` - пассивные flow из conntrack;
- `GET /api/detector/status` - текущий статус детектора;
- `GET /api/detector/history` - история детектора;
- `POST /api/detector/start` - запуск профиля детектора;
- `GET /metrics` - Prometheus metrics из Go-сервиса.

Цель этого слоя - постепенно заменить shell-CGI action'ы на один `sitewatchd`: один бинарник для OpenWrt, контейнера и агентского режима. На текущем этапе CGI и shell-скрипты остаются рабочими, а Go API используется как безопасный фундамент для миграции.

## UDP/TCP детект

Для блокировок не по доменам, а по транспортным портам, например UDP-диапазон голосовых звонков Discord, есть два режима.

Пассивный срез conntrack на роутере:
...
Активная проверка требует reflector на внешнем хосте/VPS/контейнере:

```sh
sitewatch-reflector :8096
```

На роутере или в SiteWatch-контейнере:
...
Вкладка **Детектор** объединяет ручные проверки в профили, чтобы не выбирать каждый инструмент отдельно:

- **Быстрая проверка** - проверяет URL напрямую и через v2rayA, добавляет DNS/TLS/HTTP признаки;
- **Проблемный сайт** - то же, но как отдельный профиль для сохраненной истории;
- **Discord / голос** - запускает активную UDP/TCP-проверку до reflector и пассивное conntrack-окно;
- **Полная диагностика** - проверяет URL, UDP/TCP и реальный трафик устройства за одно окно.

CLI-эквивалент:

```sh
sitewatch detector full https://youmagine.com reflector.example.org 3478,443,50000,55000 45 192.168.50.80
sitewatch-detector full https://youmagine.com reflector.example.org 3478,443,50000,55000 45 192.168.50.80
```

Статус текущего запуска хранится в `/tmp/sitewatch-detector.status`, история - в `/etc/sitewatch/detector-history.tsv`. Детектор не включает постоянное DNS-логирование сам по себе: он использует уже настроенные сборщики и короткие активные/пассивные проверки.
`sitewatch-detector` оставлен как совместимая обертка; основная реализация профилей теперь находится внутри Go-бинарника.

Пассивные наблюдения дополнительно агрегируются во вкладке **Сеть**: `источник + протокол + IP + порт`, сумма пакетов/байт, число сэмплов и последний статус. Из отчета можно скачать:

- все внешние flow в формате `proto:ip:port`;
- только подозрительные flow;
- CSV;
- подозрительные IP в формате `ip(1.2.3.4) -> proxy` для ручной вставки в v2rayA.

Локальные, multicast и broadcast цели из сетевого отчета отфильтровываются, чтобы в VPN-экспорт не попадал шум домашней сети.
...
Команда `sitewatch-dns-dump` оставлена как совместимая обертка; основной разбор tcpdump-строк и режимы `stdout`, `local`, `ingest` теперь находятся в Go-команде `sitewatch dns-dump`.

Это самый простой практичный способ увидеть обычный DNS без привязки к конкретному DNS-серверу: на OpenWrt достаточно слушать `udp/tcp port 53` на LAN-интерфейсе, и SiteWatch увидит устройства, которые спрашивают не локальный `dnsmasq` или Pi-hole, а внешний резолвер напрямую. Логи `dnsmasq`/Pi-hole надежнее показывают только запросы, которые реально прошли через этот DNS-сервер; `conntrack`, nftables-счетчики и netflow уже не содержат доменное имя.

Ограничение важное: packet dump видит только незашифрованный DNS на 53 порту. DoH, DoT, DoQ и часть QUIC-трафика не раскрывают домен в DNS-пакете, поэтому для них остаются сетевой отчет, активные UDP/TCP-проверки, DPI/TLS-эвристики и, если нужно, отдельная политика принудительного DNS на роутере.
